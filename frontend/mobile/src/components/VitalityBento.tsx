...
import { ShieldHeart, Bedtime, Timer } from 'lucide-react-native';
  ...
  <View style={styles.card}>
    <View style={styles.header}>
      <View style={styles.left}>
        <DiamondWrapper>
          <ShieldHeart size={20} color={q.accent} />
        </DiamondWrapper>
        <View>
          <Text style={styles.label}>CARBS INTAKE</Text>
          <Text style={styles.value}>150<Text style={styles.subValue}>/ 250g</Text></Text>
        </View>
      </View>
  ...
  <View style={styles.card}>
    <View style={styles.header}>
      <View style={styles.iconBox}>
        <Bedtime size={20} color={q.accent} />
      </View>
    ...
    <View style={styles.graph}>
      {[40, 60, 50, 70, 90, 40, 30].map((h, i) => (
        <View key={i} style={[styles.bar, { height: h, backgroundColor: i > 3 ? q.accent : q.accentDark + '66' }]} />
      ))}
    </View>
  ...
  <View style={[styles.card, { alignItems: 'center' }]}>
    <View style={styles.ringContainer}>
      <Svg width="120" height="120" viewBox="0 0 100 100">
        <Circle cx="50" cy="50" r="45" stroke={q.accentDeep + '40'} strokeWidth="8" fill="none" />
        <Circle 
          cx="50" cy="50" r="45" stroke={q.accent} 
          strokeWidth="8" fill="none" strokeDasharray="283" strokeDashoffset="100" 
          strokeLinecap="round" 
        />
      </Svg>
...
const styles = StyleSheet.create({
  ...
  label: { color: '#cac4d0', fontSize: 10, fontWeight: '800', letterSpacing: 1 },
  value: { color: '#fff', fontSize: 24, fontWeight: '900' },
  subValue: { fontSize: 14, color: q.accent + '80', marginLeft: 4 },
  badge: { backgroundColor: q.accent + '1A', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4, borderWidth: 1, borderColor: q.accent + '33' },
  badgeText: { color: q.accent, fontSize: 9, fontWeight: '800' },
  progressBg: { height: 16, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 8, overflow: 'hidden', borderWidth: 1, borderColor: q.accentDark + '4D' },
  progressFill: { height: '100%' },
  footer: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 8 },
  footerText: { color: q.accentDark, fontSize: 9, fontWeight: '800' },
  iconBox: { width: 40, height: 40, borderRadius: 8, backgroundColor: '#000', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: q.accent + '4D' },
  graph: { height: 80, flexDirection: 'row', alignItems: 'flex-end', justifyContent: 'space-between', gap: 4 },
  ...
  subLabel: { color: q.accent + '99', fontSize: 10, fontWeight: '800', marginTop: 4 },
});